...
// const totalAmount = totalDays*rooms?.rentperday;
  ...
  useEffect(() => {
    async function fetchRoomDetails() {
      try {
        setLoading(true);
        const data = (
          await axios.post(`${process.env.REACT_APP_API_KEY}/api/rooms/getRoomById`, { roomId: roomId })
        ).data;
  ...
  const bookRoom = async() => {
    ...
    try {
      const result = await axios.post(`${process.env.REACT_APP_API_KEY}/api/booking/bookroom`, bookingDetails)
    } catch (error) {
      console.log(error)
